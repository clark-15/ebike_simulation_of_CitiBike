# -*- coding: utf-8 -*-
"""
Runner for concentrated policy
...
with open('data/20weeks_ranked.csv') as f:
    ...
    for row in reader:
        ...
        station_demand.append(int(row[8]))
...
edock_ratio_for_concentrated_station = 0.9
num_ebike = 0.4
for total_edock_ratio in total_edock_ratio_list:
    ...
    for station in station_rank:
        if edock_num > total_edock_number: 
            break
        else:
            initial_stations[station]['edock'] = round(initial_stations[station]['ecap']*edock_ratio_for_concentrated_station)
            edock_num += initial_stations[station]['edock']
    ...
    with open('simdata/concentrated_policy_total_edock_ratio_'+str(total_edock_ratio)+'.csv','w') as f:
